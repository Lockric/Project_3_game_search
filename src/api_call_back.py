    ...
    if iter >= 0:
        for i in range (0, iter):
            byte_array = wrapper.api_request(
                'games.pb',
                f'search "{name}"; fields id, name, rating; offset {next_offset}; where rating > 0;'
            )
        ...
        while True:
            byte_array = wrapper.api_request(
                'games.pb',
                f'search "{name}"; fields id, name, rating; offset {next_offset}; where name~"{name}" & category=13 & rating > 0";'
            )
            games_message.ParseFromString(byte_array)
            if not games_message.games:
                break  # No more results, exit the loop
